...
class ErDetailWebkit(report_sxw.rml_parse,
                         CommonBalanceReportHeaderWebkit):

    def __init__(self, cursor, uid, name, context):
        super(ErDetailWebkit, self).__init__(cursor, uid, name,
                                                 context=context)
        ...
        header_report_name = ' - '.join((_('ER DETAIL'), company.name,
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        objects, new_ids, context_report_values = self. \
            compute_balance_data_er_detail(data)

        self.localcontext.update(context_report_values)

        return super(ErDetailWebkit, self).set_context(
            objects, data, new_ids, report_type=report_type)

HeaderFooterTextWebKitParser(
    'report.account.account_report_er_detail_webkit',
    'account.account',
    'addons/account_financial_report_webkit/report/templates/\
        account_report_er_detail.mako',
    parser=ErDetailWebkit)
